...
public class BienestarService {
    ...
    // Preguntas inversas corregidas (Se eliminó la 13 que es positiva)
    private static final Set<Integer> PREGUNTAS_INVERSAS = Set.of(
            2, 4, 5, 8,9, 13, 15, 20, 22, 25, 26, 27, 29, 30, 33, 34,36
    );

    // Dimensiones corregidas según tu última instrucción
    private static final Map<String, List<Integer>> DIMENSIONES = new LinkedHashMap<>();
    static {
        DIMENSIONES.put("AUTOACEPTACION",       List.of(1, 7, 13, 19, 25, 31)); // 6 ítems
        DIMENSIONES.put("RELACIONES_POSITIVAS", List.of(2, 8, 14, 20, 26, 32)); // 6 ítems
        DIMENSIONES.put("AUTONOMIA",            List.of(3, 4, 9, 10, 15, 21, 27, 33)); // 8 ítems
        DIMENSIONES.put("DOMINIO_ENTORNO",      List.of(5, 11, 16, 22, 28, 39)); // 6 ítems
        DIMENSIONES.put("PROPOSITO_VIDA",       List.of(6, 12, 17, 18, 23, 29)); // 6 ítems
        DIMENSIONES.put("CRECIMIENTO_PERSONAL", List.of(24, 30, 34, 35, 36, 37, 38)); // 7 ítems
    }
    ...
    public Participante guardarEvaluacion(Evaluacion form) {
        ...
        // Bienestar Global: Promedio de la suma de los promedios de cada dimensión
